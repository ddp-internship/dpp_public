...
export const Statistics = () => {
  ...
  // SOP: Intersection Observer untuk efek transisi masuk (Slide Up)
  useEffect(() => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (scrollRef.current) {
        ...
        let nextScroll = scrollLeft + 300; // Geser sedikit demi sedikit
  ...
  // Deteksi kartu mana yang sedang di tengah
  const handleScroll = () => {
    if (scrollRef.current) {
  ...
  if (loading) return (
    <div className="py-40 text-center">
       <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.5em] animate-pulse">Menghubungkan Database...</p>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end gap-8 mb-16 pb-10 border-b border-gray-100">
          <div className="space-y-3 text-center lg:text-left">
            ...
            <p className="text-gray-500 font-bold text-xs uppercase tracking-widest max-w-lg leading-relaxed mt-4">
              Bekerja sama dengan masyarakat di <span className="text-[#111827] font-black">{totalWilayah}</span> Wilayah binaan seluruh Indonesia.
            </p>
          </div>
        ...
        {/* --- STATS SLIDER (AUTO SCROLL & POP-OUT EFFECT) --- */}
        <div 
          ref={scrollRef}
          onScroll={handleScroll}
          className="flex overflow-x-auto gap-10 pb-20 no-scrollbar snap-x snap-mandatory pt-10"
        >
          {statsConfig.map((item, index) => (
            <div 
              key={index}
              className={`min-w-[300px] md:min-w-[340px] snap-center bg-white p-12 rounded-[3.5rem] border transition-all duration-700 flex flex-col items-center text-center relative
                ${index === activeIndex 
                  ? 'border-[#E3242B] shadow-[0_40px_80px_rgba(227,36,43,0.15)] scale-110 z-20 opacity-100' 
                  : 'border-gray-50 opacity-30 scale-90 grayscale'
                }`}
            >
              <div className={`w-20 h-20 rounded-[2.2rem] flex items-center justify-center mb-8 transition-all duration-500 ${index === activeIndex ? 'bg-[#E3242B] text-white rotate-12' : 'bg-gray-50 text-gray-300'}`}>
                <item.icon size={36} />
              </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 bg-[#111827] rounded-[4rem] p-10 md:p-14 text-white relative overflow-hidden shadow-2xl border border-white/5">
                ...
                <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="bg-white/[0.03] border border-white/10 rounded-[3.5rem] p-12 flex flex-col items-center justify-center text-center space-y-6 backdrop-blur-3xl shadow-inner">
                        <div className="w-20 h-20 bg-[#E3242B] rounded-[2.2rem] flex items-center justify-center text-white shadow-2xl shadow-red-900/50">
                             <BarChart3 size={40} />
                        </div>
                        <div className="space-y-1">
            ...
            {/* REPLACEMENT CARD: OTENTIKASI LAB DDP (Pengganti Real-time Card) */}
            <div className="lg:col-span-4 bg-[#F9FAFB] rounded-[4rem] p-12 flex flex-col justify-between items-center text-center border border-gray-100 shadow-sm relative overflow-hidden group">
                ...
                <div className="space-y-8 flex flex-col items-center">
                    <div className="w-24 h-24 bg-white rounded-[2.8rem] flex items-center justify-center shadow-xl border border-gray-50 group-hover:rotate-[360deg] transition-transform duration-1000">
                ...
                <div className="w-full space-y-4">
                    <div className="bg-white p-5 rounded-[2rem] border border-gray-100 flex items-center justify-center gap-4 shadow-sm">
