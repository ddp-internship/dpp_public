...
export const NewsPage = () => {
  ...
  return (
    <div className="bg-[#FDFDFD] min-h-screen selection:bg-[#E3242B] selection:text-white font-sans text-left">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end gap-10 md:gap-12">
            <div className="max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-1000">
              <div className="inline-flex items-center gap-4 px-5 py-2.5 bg-white/5 border border-white/10 rounded-full backdrop-blur-md">
                 ...
                 <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Pusat Media Resmi</span>
              </div>
              <div className="relative">
                  <h2 className="absolute -top-16 left-0 text-white/[0.02] text-[8rem] md:text-[12rem] font-black uppercase tracking-tighter pointer-events-none select-none hidden lg:block">PRESISI</h2>
                  <h1 className="text-5xl md:text-[100px] font-black text-white uppercase tracking-tighter leading-none relative z-10">
                    Warta <br /> 
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E3242B] to-[#ff4d4d]">Presisi</span>
                  </h1>
              </div>
            </div>
            ...
            <div className="flex bg-white/5 p-1.5 rounded-[2rem] border border-white/10 backdrop-blur-md shadow-2xl animate-in fade-in zoom-in-95 duration-1000 delay-500">
                <button onClick={() => { setActiveTab('Berita'); setPage(1); }} className={`px-8 md:px-12 py-4 rounded-[1.5rem] font-bold text-[10px] md:text-[11px] uppercase tracking-widest transition-all duration-500 ${activeTab === 'Berita' ? 'bg-[#E3242B] text-white shadow-xl scale-105' : 'text-gray-500 hover:text-white'}`}>Berita</button>
                <button onClick={() => { setActiveTab('Artikel'); setPage(1); }} className={`px-8 md:px-12 py-4 rounded-[1.5rem] font-bold text-[10px] md:text-[11px] uppercase tracking-widest transition-all duration-500 ${activeTab === 'Artikel' ? 'bg-[#111827] text-white shadow-xl' : 'text-gray-500 hover:text-white'}`}>Artikel</button>
            </div>
          </div>
        </div>
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-20"><ArrowDown size={14} className="text-white animate-bounce" /></div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          {loading ? (
            <div className="py-40 text-center">
               ...
               <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Sinkronisasi Redaksi...</p>
            </div>
          ) : (
            <div className="space-y-20 animate-in fade-in duration-1000">
                ...
                {page === 1 && items.length > 0 && (
                    <div onClick={() => setSelectedNews(items[0])} className="group cursor-pointer grid lg:grid-cols-12 gap-8 md:gap-16 items-start border-b border-gray-100 pb-16">
                        <div className="lg:col-span-8 relative rounded-[2.5rem] md:rounded-[4rem] overflow-hidden shadow-2xl bg-gray-50 aspect-video lg:aspect-auto lg:h-[550px] border-4 border-white">
                            ...
                            <div className="absolute top-6 left-6"><span className="bg-[#E3242B] text-white px-5 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest shadow-xl">UTAMA</span></div>
                        </div>
                        <div className="lg:col-span-4 space-y-6 pt-2 text-left">
                            <div className="flex items-center gap-3 text-[#E3242B] font-bold text-[10px] uppercase tracking-widest"><Tag size={14} /> {items[0].tanggal}</div>
                            <h2 className="text-3xl md:text-5xl font-black text-[#111827] tracking-tighter leading-tight group-hover:text-[#E3242B] transition-colors">{items[0].judul_artikel}</h2>
                            <p className="text-gray-500 font-medium text-sm md:text-base leading-relaxed text-left line-clamp-6">{items[0].isi_artikel}</p>
                            <button className="flex items-center gap-4 font-bold text-[10px] uppercase tracking-widest text-[#111827] group-hover:gap-8 transition-all">Baca Lengkap <ArrowUpRight size={18} className="text-[#E3242B]" /></button>
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 md:gap-16">
                    {items.slice(page === 1 ? 1 : 0).map((news) => (
                        <div key={news.id} onClick={() => setSelectedNews(news)} className="group cursor-pointer flex flex-col space-y-6 border-b md:border-none border-gray-50 pb-10 md:pb-0 text-left">
                            <div className="relative aspect-video md:aspect-[4/3] rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-gray-50 shadow-lg group-hover:shadow-2xl transition-all duration-700 border-2 border-white">
                                ...
                                <div className="absolute top-4 left-4"><span className="bg-white/90 backdrop-blur-md text-[#111827] px-4 py-1.5 rounded-xl text-[8px] font-bold uppercase tracking-widest">{news.tanggal}</span></div>
                            </div>
                            <div className="space-y-4 px-2">
                                <h3 className="text-xl md:text-2xl font-black text-[#111827] tracking-tighter leading-tight group-hover:text-[#E3242B] transition-colors line-clamp-3">{news.judul_artikel}</h3>
                                <p className="text-gray-500 font-medium text-[11px] md:text-xs leading-relaxed text-left line-clamp-3">{news.isi_artikel}</p>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      {/* --- 3. IMMERSIVE NEWS READER --- */}
      {selectedNews && createPortal(
        <div className="fixed inset-0 z-[100000] flex items-center justify-center md:p-12 text-left">
          ...
          <div className="relative bg-white w-full h-full md:max-w-6xl md:h-[90vh] md:rounded-[4rem] overflow-hidden shadow-[0_60px_120px_rgba(0,0,0,0.5)] flex flex-col md:flex-row border border-white/10 animate-in zoom-in-95 duration-500">
            ...
            <div className="md:hidden flex items-center justify-between px-6 py-4 bg-white border-b sticky top-0 z-50 text-left">
                <span className="text-[9px] font-bold text-[#E3242B] uppercase tracking-widest">{selectedNews.kategori} Release</span>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white flex flex-col h-full text-left">
                <div className="w-full aspect-video md:hidden shrink-0"><img src={selectedNews.gambar_url} className="w-full h-full object-cover" alt="Cover" /></div>
                <div className="px-6 md:px-24 py-10 md:py-20 space-y-10">
                    <div className="space-y-6">
                        <div className="hidden md:flex items-center gap-3">
                            <span className="px-4 py-1.5 bg-[#E3242B]/5 border border-[#E3242B]/10 rounded-full text-[#E3242B] font-bold uppercase text-[10px] tracking-widest">{selectedNews.kategori}</span>
                        ...
                        <h2 className="text-3xl md:text-5xl font-black text-[#111827] tracking-tighter leading-tight">{selectedNews.judul_artikel}</h2>
                        <div className="flex items-center gap-4 py-6 border-y border-gray-50">
                            ...
                            <div className="space-y-0.5">
                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Redaksi Pelaksana</p>
                                <p className="text-sm font-black text-[#111827] uppercase tracking-tight">{selectedNews.penulis}</p>
                            </div>
                        </div>
                    </div>
                    <div className="text-gray-700 text-base md:text-xl leading-relaxed font-medium text-left tracking-normal whitespace-pre-line">
                    ...
                    <div className="pt-10 border-t border-gray-100 flex items-center gap-4">
                        ...
                        <span className="text-[10px] font-bold text-[#111827] uppercase tracking-widest">Verified DDP Press Center</span>
                    </div>
                </div>
                <div className="px-6 md:px-24 py-12 bg-gray-50 border-t border-gray-100 flex items-center justify-between mt-auto">
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Laboratory Data Desa Presisi IPB University</p>
                </div>
            </div>

            <div className="hidden md:flex md:w-[35%] bg-[#111827] flex-col p-16 justify-between relative shrink-0 border-l border-white/5 overflow-hidden text-left">
                <div className="absolute top-0 right-0 w-80 h-80 bg-[#E3242B] opacity-10 blur-[100px]"></div>
                <div className="z-10 space-y-12">
                    <button onClick={() => setSelectedNews(null)} className="flex items-center gap-4 text-gray-500 hover:text-white transition-all font-bold text-[10px] uppercase tracking-widest group">
                ...
                <div className="z-10 opacity-20"><p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">Lab DDP IPB University 2026</p></div>
            </div>
          </div>
        </div>,
        document.body
