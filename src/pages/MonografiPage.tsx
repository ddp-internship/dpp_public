...
import { api } from '../api'; // Pastikan path api sesuai dengan project Anda
...
export const MonografiPage = () => {
  ...
  const handleOpenGate = (item) => {
    setSelectedBook(item);
    setShowModal(true);
  };

  const handleProcessAccess = async (e) => {
    e.preventDefault();
    try {
  ...
  return (
    <div className="bg-[#FDFDFD] min-h-screen selection:bg-[#E3242B] selection:text-white font-sans text-left">
      ...
      {/* --- 1. HERO SECTION (Sovereign Style) --- */}
          ...
          <div className="grid lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-8 space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-1000">
              <div className="inline-flex items-center gap-4 px-5 py-2.5 bg-white/5 border border-white/10 rounded-full backdrop-blur-md">
                 ...
                 <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Repositori Akademik Digital</span>
              </div>
              ...
              <p className="text-gray-400 font-medium text-sm md:text-base leading-relaxed max-w-xl border-l-4 border-[#E3242B] pl-8">
                ...
                <form onSubmit={handleSearchSubmit} className="relative group">
                    ...
                    <div className="relative flex bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-2 overflow-hidden">
                        <input 
                            ...
                            className="flex-1 bg-transparent px-6 py-4 text-white font-bold text-[10px] tracking-widest outline-none placeholder:text-gray-600 uppercase"
                            value={search}
        ...
        <div className="max-w-7xl mx-auto">
          {loading ? (
            <div className="py-40 text-center space-y-4">
               ...
               <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Synchronizing Repository...</p>
            </div>
          ) : (
            <>
                  ...
                  <div key={item.id} className="group cursor-pointer" onClick={() => handleOpenGate(item)}>
                    ...
                    <div className="mt-8 space-y-2 px-2 text-center md:text-left">
                       <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                           ...
                           <span className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Digital Archive {item.tahun}</span>
                       </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-10 lg:p-16 space-y-12">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 text-[#E3242B]">
                            <Book size={16} />
                            <span className="text-[10px] font-bold uppercase tracking-widest">Profil Monografi Desa</span>
                        </div>
                        ...
                        {[
                          ...
                          { l: 'Tahun Terbit', v: selectedBook?.tahun }
                            ...
                            <div key={idx} className="space-y-1">
                                <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">{info.l}</p>
                    ...
                    <div className="space-y-6 pb-10">
                        <div className="flex items-center gap-3">
                            ...
                            <h4 className="font-bold text-[#111827] uppercase tracking-widest text-xs">Abstrak & Ringkasan</h4>
                        </div>
            ...
            <div className="h-[50%] lg:h-full lg:w-2/5 p-10 lg:p-14 flex flex-col justify-center bg-white relative overflow-y-auto">
                ...
                <div className="mb-10 space-y-2">
                   <div className="flex items-center gap-2 text-[#E3242B]">
                      <ShieldCheck size={18} />
                      <span className="text-[10px] font-bold uppercase tracking-widest">Authorization Access</span>
                   </div>
                    ...
                    {[
                      ...
                      { l: 'Lembaga / Instansi *', k: 'instansi', t: 'text' }
                        ...
                        <div key={i} className="space-y-1">
                            <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-1">{f.l}</label>
                            <input 
                                type={f.t} 
                                required 
                                className="w-full p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold text-[#111827] tracking-tight outline-none focus:ring-4 focus:ring-red-50 transition-all" 
                                onChange={e => setVisitor({...visitor, [f.k]: e.target.value})} 
                            />
                        </div>
                    ))}
                    <div className="space-y-1">
                        <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-1">Tujuan Akses Data *</label>
                        <textarea 
                            required 
                            className="w-full p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold text-[#111827] tracking-tight outline-none focus:ring-4 focus:ring-red-50 h-24 resize-none transition-all" 
                            onChange={e => setVisitor({...visitor, keperluan: e.target.value})} 
                        />
                    </div>
                    
                    <button type="submit" className="w-full py-5 bg-[#E3242B] text-white font-bold rounded-2xl uppercase tracking-widest text-[10px] shadow-2xl shadow-red-900/20 hover:bg-[#111827] transition-all transform active:scale-95 group">
      ...
      <footer className="py-24 flex flex-col items-center gap-6 bg-white border-t border-gray-50">
           ...
           <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">
              Laboratory Data Desa Presisi IPB University
           </p>
      </footer>
